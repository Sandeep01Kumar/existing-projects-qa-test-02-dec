...
Route definitions: `GET /` is handled at `Source: server.js:L34-L36` and `GET /good-evening` at `Source: server.js:L47-L49`.

> **Unmatched paths:** any request whose path does not match `/` or `/good-evening` returns `404 Not Found` from Express's built-in default handler (`Source: server.js:L13-L14`, `Source: server.js:L34-L49`).
>
...
`GET /good-evening` returns `Good evening` (`Source: server.js:L47-L49`):
...
flowchart TD
    ...
    A -->|GET /good-evening| B
    B --> C{Route match}
    C -->|/| D["Respond: Hello world"]
    C -->|/good-evening| E["Respond: Good evening"]
...
```text
.
├── server.js           # Express app: GET / -> "Hello world", GET /good-evening -> "Good evening"
