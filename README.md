...
- **Node.js** — any modern LTS release. The reference environment used **Node v22.x**. No specific version is pinned by the project: there is no `engines` field in `package.json` and no `.nvmrc`, so any current Node LTS runs this CommonJS code.
...
This installs **Express 5.2.1**, declared as `"express": "^5.2.1"` in `package.json` (`Source: package.json:L12-L14`), and regenerates `package-lock.json` so that Express and its transitive dependencies are captured for reproducible installs.
...
The Express app registers two `GET` routes. Each handler sets a plain-text content type via `res.type('text/plain')`, which Express serves as `text/plain; charset=utf-8`.
...
> **Unmatched paths:** any request whose path does not match `/` or `/good-evening` returns `404 Not Found` from Express's built-in default handler.
>
> **Precision note:** the documented response body for `GET /` is `Hello world`, matching the wording used throughout this tutorial. For historical accuracy, the original, pre-Express implementation used Node's built-in `http` module and returned the literal `'Hello, World!\n'` — with a comma, an exclamation mark, and a trailing newline — for *every* request (`Source: original server.js:L9`, before the Express migration). The current Express version returns the exact body `Hello world` (`Source: server.js:L35`).

## Examples

With the server running (via `node server.js` or `npm start`), call each endpoint using `curl`.
...
Every request first reaches the Express application, which matches the request path against its registered routes and dispatches to the corresponding handler. Requests that match no route fall through to Express's built-in `404` handler.
...
- **`package-lock.json`** records the exact resolved dependency tree and is regenerated by `npm install`.
